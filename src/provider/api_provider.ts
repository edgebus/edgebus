...
class ApiProviderImpl extends ApiProvider {
	...
	protected async onInit(): Promise<void> {
		await FInitable.initAll(this.initExecutionContext, this._publisherApi, this._subscriberApi);
	}

	protected async onDispose(): Promise<void> {
		await FDisposable.disposeAll(this._subscriberApi, this._publisherApi);
	}
}
