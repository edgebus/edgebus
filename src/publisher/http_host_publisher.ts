...
export class HttpHostPublisher extends BasePublisher {
		...
		try {
			...
			if (this._transformers !== null) {
				for (const transformer of this._transformers) {
					messageBody = HttpHostPublisher._applyTransformer(messageBody, transformer);
				}
			}
			...
			if (this._successResponseGenerator !== null) {
				...
				res.header("EDGEBUS-MESSAGE-ID", message.messageId)
					.writeHead(200)
					.end();
			}
		} catch (e) {
	...
	private static _applyTransformer(message: any, transformer: HttpHostPublisher.Transformer): any {
...
export namespace HttpHostPublisher {
